...
draft: false
tags: ["kubernetes", "cncf", "go", "kubecolor"]
categories: ["devops"]
type: default
...
[KubeColor](https://github.com/kubecolor/kubecolor) is a thin wrapper around `kubectl` that I always used and loved.
...
KubeColor reformats the output of most `kubectl` commands to add colors. It make it *so easier* to read the output that I still don't understand that it's not more widely used. I actually [gave a lightning talk about it at the KubeCon's Cloud Native Reject europe 2024 in Paris](https://www.youtube.com/live/PWZJzjB7vso?si=CvfnmmdSZP3txENZ&t=32723) if you want a video pitch. But whatever.

One of the longest requested feature, discussed at length, was to be able to cusotmize the color scheme used for KubeColor.
...
Well, as of **version 0.3.0**, `kubecolor` now supports custom color scheme and theme, thanks to the other main contributor, [AppleJag](https://github.com/applejag), which is a talented (Go) devleoper. Jag, I can't thank you enough for all the help on this project.
...
But furthen than colors, some want to colorize some parameters differently, or use more colors to further differenciate things. 
...
And, well, when we think at inclusion it's generally genders and skin colors, and when we think of accessibility, we thing of mobility impairement, deafs and blinds. 

**Color-blindness** is usually not mentionned or taken care of. The [CNCF Website itself](https://contribute.cncf.io/accessibility/) does not mention it directly, and the only TAG (Technical Advisory Groups) in the `Accessibility` section is for hearing issues. 

Note that I'm not trying to rate any of those against the others, or trying to speak in place of the impacted persons.

Maybe because it's easier to live with color-blindness, or because people don't talk about it by shame, it's still a real problem and the numbers are huge.


## So, what to do with this ?

Well, as soon as `kubecolor` got the functionnality, I started thinking of adding one or multiple color themes for the various kind of color-blindness. 
...
First I want to clearly state I do not have any color impairement, and the work I'm trying to achieve here is based on some article readings and talking to color-blind persons. There's no scientific work on my side. 

The idea is to provide an out of the box solution to help people with color-blindness. The outcome may not be perfect or even useful and I take no responsability. It's best effort. Bare with me.
...
I also tested some chromatic progressions, trying to find a palette that could work fine:

![image](images/color-cycles-quad.png#layoutTextWidth)

Being color-blind is not just not seeing green or red, there's also quite a limitation in the color differences that are perceived, so everything from green to red where the color changes slowly for a normal eye will just be almost the same brown/yellow for a Protanopian.

My final conclusion is that it seems possible to achieve a theme that will help better differenciate the content. What we need here is having different color hues to show the difference of, mostly, good and bad situation, and color cycles when there's a table. The `turbo` color profile seems to help here.
...
Now you can use any of the themes if you're concerned by color blindness. They are:
